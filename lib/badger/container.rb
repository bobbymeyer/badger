...
module Badger
  ...
  class Container
    attr_reader :spine, :regions, :name, :tolerance, :nodes, :slot
    ...
    def initialize(source, visible: true, name: nil, slot: :ground, tolerance: 0.1)
    ...
    def rule(distance, weight: 2.0, visible: true, name: nil, slot: :ink)
    ...
    def band(outer:, inner: nil, width: nil, visible: false, name: nil, slot: :field)
      ...
      inner ||= outer - width
      add Regions::Band.new(self, outer: outer, inner: inner, visible: visible, name: name, slot: slot)
    end

    # What remains inside the innermost offset.
    def interior(inside: 0.0, visible: false, name: nil, slot: :field)
      add Regions::Interior.new(self, inside: inside, visible: visible, name: name, slot: slot)
    end
    ...
    def place(child, at:, align: :center, rotate: nil, name: nil, slot: :ink)
      ...
      add_node Node.new(child, affine, name: name, slot: slot)
    end

    # Attach a child already expressed in this container's space, such as
    # type following one of this container's own regions.
    def attach(child, name: nil, slot: :ink)
      add_node Node.new(child, Geometry::Affine.identity, name: name, slot: slot)
    end
