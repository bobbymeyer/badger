...
**The editor** is the badge page's Compose surface: the document as a tree, the drawing with its construction as the controls, an inspector for the entry selected. The core hands the editor what it needs — every piece carries the address of the entry that made it, `Output#construction` is the geometry the pieces were built on and `Output#warnings` what went wrong without failing — and the engine's `Badger::Editor` is the schema the inspector's fields are built from. A change goes to `POST /badges/:id/render` and comes back drawn; Save sends the document as JSON. A **reference**, the photograph a badge is redrawn from, goes under the drawing at a strength, moved and scaled by hand; the reference badges were measured in their photographs' own pixels, so one unit is one pixel by default. The editor's second view is the **document** as YAML, on the same line as the drawing: what is typed is drawn as it is typed, and a document that does not build is refused under the text. Every change is one history, undone with ⌘Z; anything visible can be dragged, and a number can be scrubbed by its label.
